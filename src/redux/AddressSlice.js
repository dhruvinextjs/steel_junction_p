...
export const getAddressList = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      ...
      console.log("Fetched Address List:", res.data);
...
export const updateAddress = createAsyncThunk(
  ...
  async ({ id, formData, token }, { rejectWithValue }) => {
    try {
      const res = await PutUrl.put(`/address/edit/${id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
// SET DEFAULT ADDRESS (converted ✅)
export const setDefaultAddress = createAsyncThunk(
  "address/setDefault",
  async (payload, { rejectWithValue }) => {
    try {
      const response = await PutUrl.put(
        `/address/makeDefault/${payload.selectedId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${payload.token}`,
...
const addressSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateAddress.fulfilled, (state, action) => {
        console.log("Address Update Payload:", action.payload);
